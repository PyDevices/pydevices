...
"""PyDevices WASM CLI Runner (wasm.py)

Drop-in WebAssembly CLI wrapper matching the MicroPython / CPython / Jupyter
runner interface (-m / <filename> / -c / -i / ...).

Resolves script dependencies, builds WASM gallery query URLs, checks for an active
...
Usage:
  wasm.py script.py [args...]
  wasm.py -m bouncing_balls
  wasm.py -c "import math; print(math.pi)"
  wasm.py -i
  wasm.py -m bouncing_balls --autotest --duration 3
  wasm.py -m calc_lvgl --pyodide
"""
...
def build_wasm_url(
    ...
    path = f"/pydevices-examples/gallery/{shell}"
    if query_string:
...
def run_playwright_autotest(url: str, duration: int = 2, timeout_s: int = 20) -> int:
    ...
    try:
        ...
        sys.stderr.write("wasm.py: Playwright not installed. Install playwright to use --autotest.\n")
        return 1
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if traceback_found:
                sys.stderr.write("wasm.py: Python traceback encountered during autotest:\n")
                for log in logs:
            ...
            if errors:
                sys.stderr.write(f"wasm.py: {len(errors)} page error(s) encountered:\n")
                for err in errors:
            ...
            browser.close()
            print("wasm.py: Autotest passed cleanly (0 errors).")
            return 0
        except Exception as e:
            sys.stderr.write(f"wasm.py: Autotest failed with exception: {e}\n")
            browser.close()
            return 1
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="wasm.py",
        description="PyDevices WebAssembly CLI Runner",
