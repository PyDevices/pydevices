"""Optional C accelerators for audiodev, from audiodsp when it is present.

This module exists to be a *choice*. ``audiodev/__init__.py`` and every
transport backend import no audiodsp at all -- that is what lets a headless
...
So the common claim that ``pcm_out`` needs no audiodsp in firmware holds only
...
audiodsp built in. Offline or low-rate work is fine on the portable one.

So the fast path is offered here rather than reached for there. A caller
that has audiodsp asks::
...
``audiodev.sample_out`` already requires audiodsp (it pulls
``audiocore.get_buffer``), so it may use this freely.
"""


def best_remix():
    """Return ``audiomath.remix_s16``, or ``None`` when audiodsp is absent.

    ``audiomath`` is the MicroPython usermod name; ``_audiodsp`` is the
    ...
    try:
        ...
        pass
    try:
        from _audiodsp import remix_s16
