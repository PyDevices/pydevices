...
"""Run the BLE-MIDI codec checks (``bledev_midi_codec.py``) on CPython and
MicroPython, and once per planted codec fault, each of which must fail.

The shared vectors are ``bledev_midi_vectors.json``. The port and service
checks over the fake radio are in ``bledev_contract.py`` (``midi_*``), run by
``test_bledev.py``.
"""
...
SCRIPT = _TESTS / "bledev_midi_codec.py"
PLANTS = ("rollover", "running", "sysex", "header", "realtime")
...
class BledevMidiCodec(unittest.TestCase):
    ...
    def test_micropython(self):
        ...
        if interpreter is None:
            message = "no micropython binary: the BLE-MIDI codec did NOT run on MicroPython"
